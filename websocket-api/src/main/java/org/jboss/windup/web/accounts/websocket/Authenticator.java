 ...
 */
package org.jboss.windup.web.accounts.websocket;

import static org.jboss.windup.web.accounts.websocket.internal.AuthenticationMode.CREDENTIALS;
import static org.jboss.windup.web.accounts.websocket.internal.AuthenticationMode.MESSAGE;
import static org.jboss.windup.web.accounts.websocket.internal.AuthenticationMode.TOKEN;
...
import org.jboss.windup.web.accounts.common.TokenVerifier;
import org.jboss.windup.web.accounts.common.UsernamePasswordConverter;
import org.jboss.windup.web.accounts.websocket.internal.AuthenticationMode;
import org.jboss.windup.web.accounts.websocket.internal.CachedSession;
import org.jboss.windup.web.accounts.websocket.internal.MsgLogger;
...
public class Authenticator {
    ...
    public void authenticateWithMessage(String message, Session session) throws WebsocketAuthenticationException {
        ...
        try (JsonReader jsonReader = Json.createReader(new StringReader(message))) {
            ...
            authenticate(MESSAGE, session, jsonAuth, null, null, null);
        } catch (JsonException e) {
            throw new WebsocketAuthenticationException("Invalid JSON", e);
        }
    }
    ...
    public void authenticateWithToken(String token, String personaId, Session session)
        ...
        authenticate(TOKEN, session, null, token, null, null);
    }
     ...
     * session.
     */
    public void authenticateWithCredentials(String username, String password, Session session)
            throws WebsocketAuthenticationException {
    ...
    private void authenticate(
        ...
        boolean isSessionValid = isValid(cachedSession, token, jsonAuth);
        ...
        try {
            switch (mode) {
                case CREDENTIALS:
                    cachedSession = doAuthenticationWithCredentials(username, password);
                    break;
                case MESSAGE:
                    cachedSession = doAuthenticationWithMessage(jsonAuth);
                    break;
                case TOKEN:
                    cachedSession = doAuthenticationWithToken(token);
                    break;
                default:
    ...
    private CachedSession doAuthenticationWithMessage(JsonObject jsonAuth) throws Exception {
        if (null == jsonAuth) {
            return null;
        }
        ...
        if (jsonAuth.containsKey("token")) {
            ...
            try {
                ...
                return doAuthenticationWithToken(authToken);
            } catch (ClassCastException e) {
                // this exception is thrown when token is number
                throw new WebsocketAuthenticationException("Invalid authentication token");
            }
        }
        ...
        if (null != jsonLogin) {
            ...
            return doAuthenticationWithCredentials(username, password);
        }

        return null;
    }

    private CachedSession doAuthenticationWithCredentials(String username, String password) throws
            Exception {
        ...
        return doAuthenticationWithToken(token);
    }

    private CachedSession doAuthenticationWithToken(String authToken) throws Exception {
        if (null == authToken) {
            return null;
        }
        ...
        try {
            accessToken = tokenVerifier.verify(authToken);
        } catch (Exception e) {
            throw new WebsocketAuthenticationException(e.getLocalizedMessage(), e);
        }

        if (null == accessToken.getSubject() || accessToken.getSubject().isEmpty()) {
            throw new IllegalStateException("Subject wasn't returned by the authentication server.");
        }

        long expirationTime = accessToken.getExpiration();
        expirationTime *= 1000;

        return new CachedSession(authToken, accessToken, expirationTime);
    }
    ...
    private boolean isValid(CachedSession cachedSession, String authToken, JsonObject jsonAuth) {
        ...
        // 2) the authToken is the same (ie: not a refreshed one)
